...
const PlayingXIModal = ({ isOpen, onClose, myTeam, roomId, onSuccess }) => {
  ...
  useEffect(() => {
    if (myTeam?.playingXI) {
      const savedPlayers = myTeam.playingXI.players?.map(p => p._id || p) || [];
      ...
      setCaptain(myTeam.playingXI.captain?._id || myTeam.playingXI.captain);
      setViceCaptain(myTeam.playingXI.viceCaptain?._id || myTeam.playingXI.viceCaptain);
      setWicketKeeper(myTeam.playingXI.wicketKeeper?._id || myTeam.playingXI.wicketKeeper);
    }
  }, [myTeam]);
  ...
  const validateSelection = () => {
    ...
    const xiPlayers = playingXI.map(id => getPlayerById(id)).filter(Boolean);
    const foreignPlayerCount = xiPlayers.filter(p => 
      p.country && p.country.toLowerCase() !== 'india'
    ).length;

    if (foreignPlayerCount > 4) {
      validationErrors.push(`Only 4 foreign players allowed in XI (you have ${foreignPlayerCount})`);
    }

    const wkPlayer = getPlayerById(wicketKeeper);
    if (wicketKeeper && wkPlayer && wkPlayer.role !== 'Wicket-Keeper') {
      validationErrors.push('Selected wicket keeper must have Wicket-Keeper role');
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrors([error.response?.data?.message || 'Failed to save Playing XI']);
    } finally {
      setSaving(false);
    }
  };
  ...
  const isForeign = (player) => player.country && player.country.toLowerCase() !== 'india';
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[95vw] sm:max-w-5xl max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              <h2 className="text-lg sm:text-2xl font-bold truncate">Select Playing XI</h2>
              <p className="text-purple-100 text-xs sm:text-sm mt-1">
                {myTeam.teamName} • {playingXI.filter(p => p !== null).length}/11 players
              </p>
            </div>
            <button 
              onClick={onClose} 
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mt-4 sm:mt-6">
            <div className={`bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 text-center ${playingXI.filter(p => p !== null).length === 11 ? 'ring-2 ring-green-300' : ''}`}>
              <p className="text-white/70 text-xs">Players</p>
              <p className="text-lg sm:text-xl font-bold">{playingXI.filter(p => p !== null).length}/11</p>
            </div>
            <div className={`bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 text-center ${foreignCount <= 4 ? '' : 'ring-2 ring-red-300'}`}>
              <p className="text-white/70 text-xs">Foreign</p>
              <p className="text-lg sm:text-xl font-bold">{foreignCount}/4</p>
            </div>
            <div className={`bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 text-center ${captain ? 'ring-2 ring-yellow-300' : ''}`}>
              <p className="text-white/70 text-xs">Captain</p>
              <p className="text-sm font-bold">{captain ? '✓' : 'Not Set'}</p>
            </div>
            <div className={`bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 text-center ${wicketKeeper ? 'ring-2 ring-blue-300' : ''}`}>
              <p className="text-white/70 text-xs">Wicket Keeper</p>
              <p className="text-sm font-bold">{wicketKeeper ? '✓' : 'Not Set'}</p>
            </div>
          </div>
        </div>
        ...
        {errors.length > 0 && (
          <div className="bg-red-50 border-b-2 border-red-200 p-3 sm:p-4 max-h-20 overflow-y-auto">
            {errors.map((error, index) => (
              <div key={index} className="flex items-start gap-2 text-red-700 text-xs sm:text-sm mb-2 last:mb-0">
                <AlertCircle size={14} />
                <span>{error}</span>
              </div>
            ))}
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row p-4 sm:p-6">
          {/* Playing XI Slots (Top on Mobile) */}
          <div className="lg:w-1/2 lg:pr-3 mb-4 lg:mb-0 overflow-y-auto lg:h-full max-h-[40vh] lg:max-h-none">
                    ...
                    {playerId ? (
                      <>
                        ...
                        <div className="absolute top-2 right-2 flex gap-1">
                          ...
                          >
                            <Move size={14} className="text-gray-500 hover:text-gray-700" />
                          </div>
                          <button
                        ...
                        <div className="flex flex-col items-center text-center mb-2 sm:mb-3 flex-1">
                          ...
                          {player?.soldPrice && (
                            <p className="text-xs text-green-600 font-semibold">₹{player.soldPrice} Cr</p>
                          )}
                        </div>

                        {/* Role Buttons - Stacked on Mobile */}
                      ...
                      <div className="flex flex-col items-center justify-center h-full text-gray-400">
                        ...
                        <p className="text-xs sm:text-sm font-medium">Drop player here</p>
          ...
          {/* Available Players (Bottom on Mobile) */}
          <div className="lg:w-1/2 lg:pl-3 h-full overflow-y-auto">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Available Players</h3>
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 h-full">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1 text-xs">
                          <span className="bg-gray-200 px-2 py-0.5 rounded">{player.role}</span>
                          <span className="bg-gray-200 px-2 py-0.5 rounded truncate max-w-[80px]">{player.country}</span>
                        </div>
                      </div>
                      <div className="text-right sm:ml-4 mt-1 sm:mt-0">
                        <p className="text-sm font-bold text-green-600">₹{player.soldPrice} Cr</p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-200">
                      ...
                      <div className={`w-7 h-7 rounded border-2 flex items-center justify-center ${
                        isSelected(playerId) ? 'bg-green-500 border-green-500' : 'border-gray-300 hover:border-green-400'
                      }`}>
                        {isSelected(playerId) ? (
        ...
        <div className="bg-gray-50 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 flex gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving || playingXI.filter(p => p !== null).length !== 11}
